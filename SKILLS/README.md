...
- If a skill is retired but should remain recoverable, move it to `SKILLS/_ARCHIVE/<skill-name>/` and keep its original `SKILL.md` plus a short archive note.
...
`qwen3-hf2api` and `vits-hf2api` were not present as TreasureBox skills during this normalization pass. If either is reintroduced later, keep it under `SKILLS/` only if it is active; otherwise place it under `SKILLS/_ARCHIVE/` or omit it entirely.
