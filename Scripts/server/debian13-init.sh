...
# 步驟 2.5: 檢查網路連接
echo -e "${BLUE}[INFO]${NC} 正在檢查網路連接..."
if ! ping -c 1 -W 5 google.com >/dev/null 2>&1; then
    if ! ping -c 1 -W 5 8.8.8.8 >/dev/null 2>&1; then
        echo -e "${YELLOW}警告：無法連接到網際網路。腳本將繼續執行，但可能會在安裝軟體時失敗。${NC}"
        echo -e "${BLUE}[INFO]${NC} 請確保網路連接正常後重新執行腳本。"
        ...
        echo -e "${GREEN}[OK]${NC} 網路連接正常（IP 連接成功，DNS 解析可能有問題）。"
    fi
else
    echo -e "${GREEN}[OK]${NC} 網路連接正常。"
fi
echo
...
echo -e "${BLUE}[INFO]${NC} 備份原始的 /etc/apt/sources.list..."
cp /etc/apt/sources.list /etc/apt/sources.list.backup
echo -e "${GREEN}[OK]${NC} 備份完成。"
...
if [ -f "/root/.zshrc" ]; then
    ...
    # 如果已有其他外掛，添加 docker
    elif grep -q "^plugins=(" /root/.zshrc; then
        sed -i 's/^plugins=(/plugins=(git docker /' /root/.zshrc
    else
        # 如果找不到 plugins 行，在檔案末尾添加
        echo "plugins=(git docker)" >> /root/.zshrc
    fi
    echo -e "${GREEN}[OK]${NC} Docker 外掛已啟用。"
else
